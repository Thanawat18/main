...
.form-signin {
    max-width: 800px; /* ปรับความกว้างสูงสุดตามที่ต้องการ */
    width: 100%; 
    margin: auto;
...
if (isset($_GET['o_id'])) {
    ...
    // ดึงข้อมูลจากตาราง orders_detail, products, users, orders และ order_status
    $sql = "SELECT od.od_id, od.o_id, od.p_id, od.sitem, 
                   p.p_name, p.p_price, p_detail, p.p_ext, 
                   u.u_fullname, u.u_address, u.u_tel, 
                   o.o_total, o.o_date, s.o_status
            FROM orders_detail od
            JOIN product p ON od.p_id = p.p_id
            JOIN orders o ON od.o_id = o.o_id
            JOIN users u ON o.u_id = u.u_id
            JOIN order_status s ON o.s_id = s.s_id
            WHERE od.o_id = ?";
    ...
    if ($orderDetails = $result->fetch_assoc()) {
        ...
        $totalPrice = $orderDetails['p_price'] * $orderDetails['sitem'];

        // กำหนดเส้นทางของรูปภาพ
        $img = "images/" . $orderDetails['p_id'] . "." . $orderDetails['p_ext'];
        ?>
        <!DOCTYPE html>
        <html lang="th">
        <head>
        ...
        <body>
                    <div class="card-body">
                        ...
                        <td><img src="<?= $img; ?>" width="120"></td> <!-- แสดงรูปภาพ -->
                        <p><strong>ชื่อหนังสือ:</strong> <?= $orderDetails['p_name']; ?></p>
                        <p><strong>รายละเอียด:</strong> <?= $orderDetails['p_detail']; ?></p>
                        <p><strong>ราคา:</strong> <?= $orderDetails['p_price']; ?> บาท</p>
                        <p><strong>จำนวนที่สั่ง:</strong> <?= $orderDetails['sitem']; ?></p>
                        <p><strong>ราคารวม:</strong> <?= $totalPrice; ?> บาท</p>
                        <hr>
                        <p><strong>ชื่อผู้สั่ง:</strong> <?= $orderDetails['u_fullname']; ?></p>
                        <p><strong>ที่อยู่:</strong> <?= $orderDetails['u_address']; ?></p>
                        <p><strong>เบอร์โทร:</strong> <?= $orderDetails['u_tel']; ?></p>
                        <hr>
                        <p><strong>วันที่สั่งซื้อ:</strong> <?= $orderDetails['o_date']; ?></p>
                        <p><strong>สถานะคำสั่งซื้อ:</strong> <?= $orderDetails['o_status']; ?></p>
                        <hr>
                    </div>
<div class="text-end">
    <button type="button" class="btn btn-warning" onclick="window.location.href='user_orders.php'">
