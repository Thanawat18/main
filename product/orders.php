...
<?php
        ...
        ?>
            <tr>
<!-- ปุ่มสำหรับเปิด modal การแก้ไข -->
<td>
    <a href="#" class="btn btn-warning btn-sm" onclick="editOrder('<?= $data['o_id']; ?>')">
        <i class="bi bi-pen"></i> Edit
    </a>
</td>

<!-- Modal สำหรับการแก้ไขข้อมูลคำสั่งซื้อ -->
<div class="modal fade" id="editOrderModal" tabindex="-1" aria-labelledby="editOrderModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editOrderModalLabel">แก้ไขสถานะคำสั่งซื้อ</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="editOrderForm">
                    <input type="hidden" id="editOId" name="o_id">
                    
                    <!-- สถานะคำสั่งซื้อ -->
                    <div class="mb-3">
                        <label for="editSId" class="form-label">สถานะคำสั่งซื้อ</label>
                        <select name="s_id" class="form-control" id="editSId" required>
                            <?php
                            $sql = "SELECT s_id, o_status FROM order_status";
                            $result = mysqli_query($conn, $sql);
                            if (mysqli_num_rows($result) > 0) {
                                while ($row = mysqli_fetch_array($result)) {
                                    echo "<option value='".$row['s_id']."'>".$row['o_status']."</option>";
                                }
                            } else {
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" onclick="updateOrder()">บันทึก</button>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="editOrderResultModal" tabindex="-1" aria-labelledby="editOrderResultModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editOrderResultModalLabel">ผลลัพธ์การแก้ไขสถานะคำสั่งซื้อ</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" onclick="window.location.href='orders.php'"></button>
            </div>
            <div class="modal-body" id="editOrderResultMessage">
                <!-- ข้อความผลลัพธ์จะถูกแสดงที่นี่ -->
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" onclick="window.location.href='orders.php'">ปิด</button>
            </div>
        </div>
    </div>
</div>
...
function editOrder(o_id) {
    // ส่ง request เพื่อดึงข้อมูล
    fetch('get-order-data.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({
            o_id: o_id  // ส่ง o_id ไปยัง PHP
        })
    })
    ...
    .then(data => {
        ...
        if (data.success) {
            // กรอกข้อมูลลงในฟอร์มแก้ไขเฉพาะสถานะ
            document.getElementById('editOId').value = data.data.o_id;
            document.getElementById('editSId').value = data.data.s_id; // ใช้ s_id เพื่ออัพเดท select สถานะ
...
function updateOrder() {
    ...
    fetch('update-order.php', {  // เปลี่ยนไปใช้ไฟล์ PHP ที่อัปเดตข้อมูลคำสั่งซื้อ
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
    .then(data => {
        ...
        var editOrderResultMessage = document.getElementById('editOrderResultMessage');
        if (data.success) {
            editOrderResultMessage.textContent = 'แก้ไขสถานะคำสั่งซื้อสำเร็จ!';
        } else {
            editOrderResultMessage.textContent = 'เกิดข้อผิดพลาดในการแก้ไขข้อมูล: ' + data.message;
        }

        // แสดง modal ผลลัพธ์
        var editOrderResultModal = new bootstrap.Modal(document.getElementById('editOrderResultModal'));
        editOrderResultModal.show();
    })
    .catch(error => {
        console.error('Error:', error);
        alert('เกิดข้อผิดพลาดในการเชื่อมต่อ');
...
<td>
<button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#staticBackdrop" onclick="deleteItem('<?= $data['o_id']; ?>')">
...
<div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="staticBackdropLabel">ยืนยันการลบคำสั่งซื้อ</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                คุณต้องการลบคำสั่งซื้อนี้ออกจากรายการหรือไม่?
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">ยกเลิก</button>   
...
<div class="modal fade" id="deleteresultModal" tabindex="-1" aria-labelledby="deleteresultModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="deleteresultModalLabel">ผลลัพธ์การลบคำสั่งซื้อ</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" onclick="window.location.href='orders.php'"></button>
            </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" onclick="window.location.href='orders.php'" data-bs-dismiss="modal">ปิด</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function deleteItem(o_id) {
        deleteId = o_id; // บันทึก ID ของคำสั่งซื้อที่จะลบ
    }

    function confirmDelete() {
        // ใช้ fetch ส่ง request ไปยังไฟล์ PHP ที่จะลบคำสั่งซื้อ
        fetch('delete-order.php', {  // เปลี่ยนไปใช้ไฟล์ที่เหมาะสมสำหรับลบคำสั่งซื้อ
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({
                o_id: deleteId  // ส่ง o_id ไปยัง PHP
            })
        })
        .then(response => response.json())
        .then(data => {
            ...
            if (data.success) {
                deleteresultMessage.textContent = 'ลบคำสั่งซื้อสำเร็จ!';
            } else {
                deleteresultMessage.textContent = 'เกิดข้อผิดพลาดในการลบคำสั่งซื้อ: ' + (data.message || 'ไม่ทราบสาเหตุ');
            }
...
<!-- ปุ่ม Detail -->
<td>
<a href="order-detail.php?o_id=<?= $data['o_id']; ?>" class="btn btn-success btn-sm">
