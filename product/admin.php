...
<h1>Admin 
    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal">Add <i class="bi bi-plus"></i></button>
</h1>
...
<?php
  ...
  ?>
            <tr>
<td>
    <a href="#" class="btn btn-warning btn-sm" onclick="editItem('<?= $data['a_id']; ?>', '<?= $data['a_name']; ?>', '<?= $data['a_email']; ?>', '<?= $data['a_username']; ?>')">
        <i class="bi bi-pen"></i> Edit
    </a>
</td>

<!-- Modal สำหรับการแก้ไขข้อมูลแอดมิน -->
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editModalLabel">แก้ไขข้อมูลแอดมิน</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="editForm">
                    <input type="hidden" id="editAId" name="a_id">
                    <div class="mb-3">
                        <label for="editAName" class="form-label">ชื่อแอดมิน</label>
                        <input type="text" class="form-control" id="editAName" name="a_name" required>
                    </div>
                    <div class="mb-3">
                        <label for="editAEmail" class="form-label">อีเมลแอดมิน</label>
                        <input type="email" class="form-control" id="editAEmail" name="a_email" required>
                    </div>
                    <div class="mb-3">
                        <label for="editAUsername" class="form-label">ชื่อผู้ใช้</label>
                        <input type="text" class="form-control" id="editAUsername" name="a_username" required>
                    </div>
                    <div class="mb-3">
                        <label for="editAPassword" class="form-label">รหัสผ่าน</label>
                        <input type="password" class="form-control" id="editAPassword" name="a_password">
                        <small class="form-text text-muted">กรุณากรอกเพื่อเปลี่ยนรหัสผ่าน หากไม่ต้องการเปลี่ยนให้เว้นว่าง</small>
                    </div>
                </form>
            </div>
...
<div class="modal fade" id="editResultModal" tabindex="-1" aria-labelledby="editResultModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editResultModalLabel">ผลลัพธ์การแก้ไขแอดมิน</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body" id="editResultMessage">
                <!-- ข้อความผลลัพธ์จะถูกแสดงที่นี่ -->
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" onclick="window.location.href='admin.php'">ปิด</button>
            </div>
        </div>
    </div>
</div>
...
<script>
function editItem(a_id, a_name, a_email, a_username) {
    // แสดงข้อมูลใน modal
    document.getElementById('editAId').value = a_id;
    document.getElementById('editAName').value = a_name;
    document.getElementById('editAEmail').value = a_email;
    document.getElementById('editAUsername').value = a_username;
...
function updateAdmin() {
    ...
    fetch('update-admin.php', {  // เปลี่ยนไปใช้ไฟล์ที่เหมาะสมสำหรับอัปเดตแอดมิน
        method: 'POST',
        body: formData
    })
    ...
    .then(data => {
        ...
        if (data.success) {
            editResultMessage.textContent = 'แก้ไขข้อมูลแอดมินสำเร็จ!';
        } else {
            editResultMessage.textContent = 'เกิดข้อผิดพลาดในการแก้ไขข้อมูล: ' + data.message;
        }

        // แสดง modal ผลลัพธ์
        var editResultModal = new bootstrap.Modal(document.getElementById('editResultModal'));
...
<!-- ปุ่มสำหรับลบแอดมิน -->
<td>
    <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#staticBackdrop" onclick="deleteItem('<?= $data['a_id']; ?>')">
...
<div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="staticBackdropLabel">ยืนยันการลบแอดมิน</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                คุณต้องการลบแอดมินนี้ออกจากรายการหรือไม่?
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">ยกเลิก</button>   
...
<div class="modal fade" id="deleteresultModal" tabindex="-1" aria-labelledby="deleteresultModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="deleteresultModalLabel">ผลลัพธ์การลบแอดมิน</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" onclick="window.location.href='admin.php'" data-bs-dismiss="modal">ปิด</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function deleteItem(a_id) {
        deleteId = a_id; // บันทึก ID ของแอดมินที่จะลบ
    }

    function confirmDelete() {
        // ใช้ fetch ส่ง request ไปยังไฟล์ PHP ที่จะลบแอดมิน
        fetch('delete-admin.php', {  // เปลี่ยนไปใช้ไฟล์ที่เหมาะสมสำหรับลบแอดมิน
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({
                a_id: deleteId  // ส่ง a_id ไปยัง PHP
            })
        })
        .then(response => response.json())
        .then(data => {
            ...
            if (data.success) {
                deleteresultMessage.textContent = 'ลบแอดมินสำเร็จ!';
            } else {
                deleteresultMessage.textContent = 'เกิดข้อผิดพลาดในการลบแอดมิน: ' + (data.message || 'ไม่ทราบสาเหตุ');
            }
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="exampleModalLabel">เพิ่มแอดมิน</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="adminForm">
...
<div class="modal fade" id="addresultModal" tabindex="-1" aria-labelledby="addresultModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="addresultModalLabel">ผลลัพธ์การเพิ่มแอดมิน</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
    ...
    if (form.checkValidity()) {
        ...
        .then(data => {
			...
			// ปิดโมเดลแรก
                ...
                modal.hide(); 
            // แสดงผลลัพธ์ใน Modal
            var addresultMessage = document.getElementById('addresultMessage');
            if (data.success) {
                addresultMessage.textContent = 'เพิ่มแอดมินสำเร็จแล้ว';
            } else {
                addresultMessage.textContent = 'เกิดข้อผิดพลาดในการเพิ่มแอดมิน: ' + data.error;
            }
            var addresultModal = new bootstrap.Modal(document.getElementById('addresultModal'));
            addresultModal.show(); // แสดง modal ผลลัพธ์
        }).catch(error => {
            console.error('Error:', error);
        });
    } else {
