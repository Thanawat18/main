...
<div class="container border">
   <h1>Manage Category <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal">Add <i class="bi bi-plus"></i></button></h1>
<?php
...
if (isset($_POST['Submit'])) {
    ...
    // SQL สำหรับการบันทึกข้อมูลลงตาราง category
    $sql = "INSERT INTO `category` (`c_id`, `c_name`) VALUES (NULL, '{$name}')";

    // ตรวจสอบว่าบันทึกสำเร็จหรือไม่
    if (mysqli_query($conn, $sql)) {
        $modalTitle = 'Success';
        $modalBody = 'เพิ่มหมวดหมู่สำเร็จ';
    } else {
        $modalTitle = 'Error';
    ...
    <table id="myTable" class="table table-striped table-hover" style="width:100%">
        ...
        <tbody>
            ...
            ?>
                <tr>
<td>
    <a href="#" class="btn btn-warning btn-sm" onclick="editItem('<?= $data['c_id']; ?>', '<?= $data['c_name']; ?>')">
        <i class="bi bi-pen"></i> Edit
    </a>
</td>
<td>
    <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#staticBackdrop" onclick="deleteItem('<?= $data['c_id']; ?>')">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="addCategoryForm" action="" method="post">
                        <div class="mb-3">
                            <label for="categoryName" class="form-label">ชื่อประเภทหนังสือ</label>
                            <input type="text" class="form-control" id="categoryName" name="category_name" required>
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
                    <button type="submit" name="Submit" form="addCategoryForm" class="btn btn-primary">บันทึก</button>               
    ...
    <div class="modal fade" id="addresultModal" tabindex="-1" aria-labelledby="addresultModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="addresultModalLabel"><?php echo $modalTitle; ?></h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <?php echo $modalBody; ?>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" onclick="window.location.href='managecat.php'">ปิด</button>
                </div>
            </div>
        </div>
    </div>
<script>
        // เช็คว่ามีการตั้งค่าชื่อ modal หรือไม่ ถ้ามีก็แสดง modal
        <?php if ($modalTitle !== ''): ?>
            $(document).ready(function() {
...
<div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="staticBackdropLabel">ยืนยันการลบหมวดหมู่</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                คุณต้องการลบหมวดหมู่นี้ออกจากรายการหรือไม่?
            </div>
            <div class="modal-footer">
...
<div class="modal fade" id="deleteresultModal" tabindex="-1" aria-labelledby="deleteresultModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="deleteresultModalLabel">ผลลัพธ์การลบหมวดหมู่</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" onclick="window.location.href='managecat.php'" data-bs-dismiss="modal">ปิด</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function deleteItem(c_id) {
        deleteId = c_id;
    }

    function confirmDelete() {
        // ใช้ fetch ส่ง request ไปยังไฟล์ PHP ที่จะลบหมวดหมู่
        fetch('delete-category.php', {  // เปลี่ยนไปใช้ไฟล์ที่เหมาะสมสำหรับลบหมวดหมู่
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({
                c_id: deleteId  // ส่ง c_id ไปยัง PHP
            })
        })
        ...
        .then(data => {
            ...
            if (data.success) {
                deleteresultMessage.textContent = 'ลบหมวดหมู่สำเร็จ!';
            } else {
                deleteresultMessage.textContent = 'เกิดข้อผิดพลาดในการลบหมวดหมู่: ' + (data.message || 'ไม่ทราบสาเหตุ');
            }
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editCategoryForm" action="" method="post">
                    <input type="hidden" id="editCategoryId" name="c_id">
                    <div class="mb-3">
                        <label for="editCategoryName" class="form-label">ชื่อประเภทหนังสือ</label>
                        <input type="text" class="form-control" id="editCategoryName" name="category_name" required>
                    </div>
                </form>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-primary" onclick="updateCategory()">บันทึก</button>
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">ปิด</button>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="editresultModal" tabindex="-1" aria-labelledby="editresultModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editresultModalLabel">ผลลัพธ์การแก้ไขหมวดหมู่</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
...
<script>
function editItem(c_id, c_name) {
    document.getElementById('editCategoryId').value = c_id;
    document.getElementById('editCategoryName').value = c_name;
...
function updateCategory() {
    const c_id = document.getElementById('editCategoryId').value;
    ...
    .then(data => {
        ...
        // แสดงผลลัพธ์ใน modal ใหม่
        var editresultMessage = document.getElementById('editresultMessage');
        if (data.success) {
            editresultMessage.textContent = 'แก้ไขหมวดหมู่สำเร็จ!';

            // อัปเดตข้อมูลในตารางแสดงผล
            const rowToUpdate = document.querySelector(`tr[data-id='${c_id}']`); // ค้นหาแถวที่ต้องการอัปเดต
            if (rowToUpdate) {
                const nameCell = rowToUpdate.querySelector('.category-name'); // สมมติว่า class นี้ใช้แสดงชื่อหมวดหมู่
                nameCell.textContent = category_name; // อัปเดตชื่อหมวดหมู่
            }
        } else {
            editresultMessage.textContent = 'เกิดข้อผิดพลาดในการแก้ไข: ' + (data.message || 'ไม่ทราบสาเหตุ');
        }
